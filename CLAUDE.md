...
## Keywords
Telegram, parser, contact-extraction, LLM, OpenRouter, Gemini-Flash, Excel, desktop-app, tkinter, httpx, openpyxl

## Annotation
Desktop application for extracting contact data (names, phones, emails, addresses) from Telegram Desktop chat exports (JSON) using LLM (OpenRouter/Gemini Flash), with deduplication and Excel output. Designed for non-technical users, runs locally without a server.
